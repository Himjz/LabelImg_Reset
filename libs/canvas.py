...
class Canvas(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.scale = 1.0
        # 修复：将overlay_color初始化为透明的QColor对象而非None
    ...
    def transform_pos(self, point):
        ...
        result = QPointF(
            scaled_point.x() - offset.x(),
            scaled_point.y() - offset.y()
        )
        return result
    ...
    def offset_to_center(self):
        ...
        return QPointF(
            center.x() / self.scale - self.pixmap.width() / 2.0,
            center.y() / self.scale - self.pixmap.height() / 2.0
        )
    ...
    def wheelEvent(self, ev):
        ...
        mods = ev.modifiers()
        # 修正：使用 & 运算符判断修饰键组合，无需转换为 int
        if (mods & (Qt.ControlModifier | Qt.ShiftModifier)) == (Qt.ControlModifier | Qt.ShiftModifier) and v_delta:
            self.lightRequest.emit(v_delta)
        elif (mods & Qt.ControlModifier) == Qt.ControlModifier and v_delta:
